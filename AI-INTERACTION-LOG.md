...
4. **What I deliberately wrote by hand**: The `TransitionToFault` retry loop (I wanted to reason about `serialization_failure` mapping directly), the `VehicleLockMap` (so I could think about the double-checked locking pattern), the ADR (the whole point of an ADR is that *I* believe each decision, not the model), and the prior-experience disclosure (judgement call about tone).

5. **What I'd do differently next time**: Insist on a "plan first, no code" turn before every non-trivial implementation prompt. The two clearest failures (Vite config and the late-binding closure) both happened when I let the agent jump straight to code. The times I forced a plan turn (anomaly rules, isolation strategy) produced output I could land almost as-is.
